...
class PointCloudMLP(nn.Module):
    def __init__(self, dim):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(dim, 128),  # Flattened input size
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 39)  # Output size: 13 * 3
        )
    ...
    def forward(self, x):
        ...
        x = self.mlp(x)
        x = x.view(-1, 13, 3)  # Reshape to (13, 3)
        return x
...
class PointTransformerV3(PointModule):
    ...
    def forward(self, data_dict):
        ...
        batch_size = max(point['batch']).item() + 1
        point['feat'] = point['feat'].reshape(batch_size, -1, point['feat'].shape[-1])  # B, 1024, 64

        point['feat'] = torch.mean(point['feat'], dim=1, keepdim=True)  # B, 1, 64

        mlp_head_dim = point['feat'].shape[-1]*point['feat'].shape[-2]

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        mlp_head = PointCloudMLP(dim=mlp_head_dim).to(device)
        keypoints = mlp_head(point['feat'])
...
if __name__ == "__main__":
    ...
    # import spconv.pytorch as spconv
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Create a dummy data_dict for testing
    data_dict = {
        "feat": torch.randn(4*1024, 5).to(device),  # Random features for 1024 points
        "coord": torch.randn(4*1024, 3).to(device),  # Random grid coordinates
        # "batch": torch.cat([torch.full((1024,), i, dtype=torch.int64) for i in range(4)]).to(device),
        "offset": torch.tensor([1024, 1024*2,1024*3,1024*4]).to(device),
    ...
    # model = PointTransformerV3(order=( "hilbert", "hilbert-trans"))
