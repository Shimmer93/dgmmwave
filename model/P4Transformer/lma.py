...
class LMA_P4T(nn.Module):
    ...
    def forward_train(self, x):
        ...
        # y_lidar, vis_lidar = self.dec_lidar(feat_mem_lidar, x_lidar)
        # y_mmwave, _ = self.dec_mmwave(feat_mem_mmwave, x_mmwave)
        y_lidar = self.dec_lidar(feat_mem_lidar)
        y_mmwave = self.dec_mmwave(feat_mem_mmwave)
        y_mmwave0, y_mmwave1 = torch.chunk(y_mmwave, 2, dim=0)
        y_transferred = self.dec_mmwave(feat_mem_transferred)
        # y_transferred, vis_transferred = self.dec_mmwave(feat_mem_transferred, x_lidar)

        return y_lidar, y_transferred, y_mmwave0, y_mmwave1, \
               l_rec_lidar, l_rec_mmwave
    ...
    def forward_inference(self, x):
        ...
        # y, _ = self.dec_mmwave(feat_mem, x)
        return y
